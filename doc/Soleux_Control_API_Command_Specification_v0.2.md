...
Status: Relay Module transport implemented; full command catalogue remains the target contract
...
Compatibility: Relay legacy port is AT-only; the Windows Relay path uses the Control API
Implementation status: Relay Module firmware now exposes the implemented protocol 2 command subset on the configured legacy TCP port plus 3 and through POST /api/v1/command over HTTP or HTTPS. The broader version 3 catalogue in this document remains the target contract; clients must use hello/capability data and must not assume every catalogued action is available yet.

# Document purpose
This specification defines one transport-neutral command model for Soleux device firmware, the Windows application, the upcoming mobile application and third-party HTTP/HTTPS integrations. Relay control no longer uses app-only J: messages. The configured legacy port remains available for AT clients, while modern Relay clients use the separate Control API transport.
## Normative rules
...
## Implemented Relay Module HTTP/HTTPS endpoint
The Relay Module exposes one transport-neutral command endpoint. Every action currently available on the JSON TCP Control API is accepted at this endpoint, including Relay state/configuration operations, page configuration and actions, system-log pagination and deletion, and file-transfer operations.
...
| Client access | Existing HTTP API IP/MAC rules apply. The direct socket peer address is used; forwarding headers are not trusted for authorization. |
...
```json
{"protocol":2,"id":"mobile-1","ok":true,"result":{"device":"relay_module","api_port":5008}}
```
## Verification commands
Run the source-level contract tests from the Soleux App project root on a developer computer with Python 3:
python -m unittest discover -s "Device Firmware\RelayModule\tests" -p "test_*.py" -v
After deploying web-service-redis.py and restarting WebService, run this on the Relay Module to verify the live HTTP endpoint:
...
Compatibility rule: The J: prefix is not accepted on the Control API or the legacy Relay port. The standard Windows Relay path uses JSON on legacy port + 3 and does not send AT commands or fall back to the AT-only legacy port. Adding the HTTP/HTTPS endpoint does not change existing AT behavior.
...
The implemented Relay HTTP/HTTPS command endpoint applies the existing API IP/MAC allow rules to the direct socket peer. If API filtering is disabled in device settings, requests are allowed from any reachable address. Deployments should restrict HTTP to trusted local networks and use HTTPS for credentials, backup/restore, firmware or remote/mobile access. Tokens and secrets must never be written to normal logs.
...
| api_access | IP/MAC allow rules, HTTP enabled, HTTPS enabled, authentication target, rate-limit target |
...
* Relay firmware exposes the implemented protocol 2 command subset on the configured legacy TCP port + 3 (5008 by default).
* Relay firmware exposes the same implemented commands through POST /api/v1/command on HTTP or HTTPS.
* Discovery and heartbeat identity report the calculated Control API port so clients do not hardcode 5008.
* The standard Windows Relay path uses the JSON Control API and does not send AT commands or use the legacy port.
* The configured legacy Relay port remains AT-only for existing third-party or service integrations.
...
Next protocol milestone: Promote the implemented Relay protocol 2 subset toward the version 3 target only after command names, authentication, permissions, event transport, output delay units, schedule semantics and cross-device capabilities are confirmed.
